...
Skill 核心是领域无关的编译引擎。垂直领域的专业性通过**三层**插件注入，全部可选、可叠加：

| 层 | 载体 | 作用 | 能阻断写入？ |
|----|------|------|------|
| **种子（seed）** | `seeds/{name}.md` | 冷启动词表：标准术语、关系模板、禁混规则 | 否（建议性） |
| **校验器（validator）** | `validators/{name}.md` | Lint 时完整性检查：关系合法性、必要关系完整性 | 否（仅 lint 期） |
| **治理（governance）** | `references/governance/{name}.md` | candidate 抽取与 wiki 写入之间的 route/conflict 前置门 | **能**（route-before-store） |

没有插件，wiki 自由生长，适合探索性研究。有了插件，wiki 从行业标准起步，概念命名规范、关系结构清晰、逻辑缺口可检测；带 governance 时还多一层"agent 提议 → 用户审核 → 接纳"的 review 队列。

**社区可贡献**：为你的垂直领域写一个 seed / validator / governance 模块（markdown） —— 声明 20-50 个核心术语和禁混规则，或者接一个治理后端，该领域的 wiki 就能复用。

当前可用：
- `seeds/fibo-pensions.md` — 企业年金/养老金（基于 FIBO 标准）
- `validators/fibo-mcp.md` — FIBO SPARQL 逻辑校验（627K 推理三元组）
- `references/governance/ontology-mem.md` — Route-before-store + 5 条冲突规则 + append-only ledger（适用场景：错了有真实代价的长期记忆 —— 金融 / 医疗 / 合规 / 多人协作）

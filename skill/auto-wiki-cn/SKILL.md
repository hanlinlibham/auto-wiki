...
description: |
  ...
  触发词：「知识模式」「带着知识回答」「打开 wiki」
  「ingest」「编译」「整理这篇」「wiki」「知识库」「lint」「检查 wiki」
...
Agent: 已进入知识模式。当前 wiki：22 页 / 8 数据点 / 2 处 contested。
       接下来的问题我会先查 wiki 再回答。
...
| 模式 | 触发 | Agent 做什么 |
|------|------|-------------|
| **知识模式** | 「知识模式」「打开 wiki」「带着知识回答」 | 加载 wiki 上下文，后续所有问题先查 wiki 再回答 |
...
知识模式 vs query 的区别：query 是单次操作（问一个问题，查一次 wiki）。知识模式是持续状态——进入后，这轮对话里的每个问题都先过 wiki。
...
用户说"知识模式"、"打开 XX wiki"、"带着知识回答"时触发。
...
5. **向用户报告**：
   ...
   接下来的问题我会先查 wiki 再回答。说"退出知识模式"恢复正常。
   ```
...
用户说"退出知识模式"、"正常聊"、或切换到其他操作（ingest/lint）时退出。

...
2. 如果 meta.yaml 声明了 seed → 读取种子文件，对照词表：
   ...
   - 该实体属于哪个标准类别？→ 放入对应的类型目录（`机构/` `工具/` `指标/` `机制/` `事件/` …，以本域 `_ontology.md` 为准）
   - 是否触碰禁混规则？→ 在页面中明确区分
3. 正常执行 ingest 后续步骤
```
...
→ 创建 `机构/某银行.md`（机构页面；中文 slug = 文件名 = wikilink = data.db 主键）
→ 在关系中标注 bank-x 担任 trustee 角色
→ 不创建 `机构/某银行-受托人.md`（机构 ≠ 角色，遵守禁混规则——同一机构可同时任受托人与账管人，拆页会把实体打碎）
```

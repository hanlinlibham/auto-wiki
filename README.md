...
>
> ℹ️ `auto-wiki-cn` is the only maintained edition (**v0.4.4+survey.2.scaling.2**, eight modes). The protocol
...
>
> ℹ️ 中文版是唯一在维护的版本（**v0.4.4+survey.2.scaling.2**，八模式）。英文版已于 2026-08-12
...
| 版本 | 目录 | 状态 |
|------|------|------|
| 中文 | `skill/auto-wiki-cn/` | **v0.4.4+survey.2.scaling.2** — 在维护。八模式（survey 存量勘察、init 访谈建库、source 取材）、五试预检、L1/L2 分层索引、实例配置解耦、可运行示例 |
...
|---------|-----------|--------|
| Chinese | `skill/auto-wiki-cn/` | **v0.4.4+survey.2.scaling.2** — maintained. Eight modes (this edition adds `survey` structure-only recon of existing notes; 0.4.x added `init` and `source`), five-check precheck, L1/L2 tiered indexes, decoupled instance config, runnable example |
